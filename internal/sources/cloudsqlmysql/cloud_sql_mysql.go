...
func (s *Source) pool(ctx context.Context) (*sql.DB, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*sql.DB, error) {
		pool, err := initCloudSQLMySQLConnectionPool(ctx, s.tracer, s.Name, s.Project, s.Region, s.Instance, s.IPType.String(), s.User, s.Password, s.Database, s.ReadOnly)
		if err != nil {
...
// MySQLPool reports the pool if one has been made. It is the type
// discriminator tools assert on; callers needing a guaranteed-live pool must
// use MySQLPoolContext, since a deferred source has not connected yet.
