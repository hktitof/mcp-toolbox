...
func (s *Source) pool(ctx context.Context) (*pgxpool.Pool, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*pgxpool.Pool, error) {
		pool, err := initAlloyDBPgConnectionPool(ctx, s.tracer, s.Name, s.Project, s.Region, s.Cluster, s.Instance, s.IPType.String(), s.User, s.Password, s.Database, s.ReadOnly)
		if err != nil {
