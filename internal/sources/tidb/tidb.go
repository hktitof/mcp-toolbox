...
func (s *Source) pool(ctx context.Context) (*sql.DB, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*sql.DB, error) {
		pool, err := initTiDBConnectionPool(ctx, s.tracer, s.Name, s.Host, s.Port, s.User, s.Password, s.Database, s.UseSSL)
		if err != nil {
...
// TiDBPool reports the pool if one has been made. It is the type discriminator
// tools assert on; a deferred source has not connected yet, so it reports nil
