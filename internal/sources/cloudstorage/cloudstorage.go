...
// client returns the storage client, creating it on first use.
func (s *Source) client(ctx context.Context) (*storage.Client, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*storage.Client, error) {
		client, err := initGCSClient(ctx, s.tracer, s.Name, s.Project)
		if err != nil {
...
// StorageClient reports the client if one has been made. No tool calls it
// today; it exists as the type discriminator, so a deferred source that has
// not connected yet reports nil rather than paying for a connect.
