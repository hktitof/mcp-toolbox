...
// clientSet groups the handles this source builds in one connect. Which of
// them is populated depends on the authentication mode, exactly as before.
...
// clients returns the Cloud Logging Admin handles, creating them on first use.
func (s *Source) clients(ctx context.Context) (*clientSet, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*clientSet, error) {
		...
		client, tokenSource, err := initLogAdminConnection(ctx, s.tracer, s.Name, s.Project, s.ImpersonateServiceAccount)
		if err != nil {
...
// LogAdminClient reports the ADC-backed client if one has been made; a
...
// LogAdminClientCreator reports the per-token client factory if one has been
// made.
