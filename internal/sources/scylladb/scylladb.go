...
// session returns the ScyllaDB session, creating it on first use.
func (s *Source) session(ctx context.Context) (*gocql.Session, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*gocql.Session, error) {
		session, err := initScyllaDBSession(ctx, s.tracer, s.Config)
		if err != nil {
...
// ScyllaDBSession reports the session if one has been made. It is the type
