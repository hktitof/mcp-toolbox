...
// client returns the MongoDB client, creating it on first use.
func (s *Source) client(ctx context.Context) (*mongo.Client, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*mongo.Client, error) {
		client, err := initMongoDBClient(ctx, s.tracer, s.Name, s.Uri)
		if err != nil {
...
// MongoClient reports the client if one has been made. It is the type
