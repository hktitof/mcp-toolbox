...
// httpClient bundles the two things the source builds together: the guarded
// client and the headers every request starts from. They are produced by the
// same pass over the config, so one ConnectOnce holds both.
...
// client returns the guarded HTTP client and its default headers, building
// them on first use.
func (s *Source) client(ctx context.Context) (*httpClient, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*httpClient, error) {
		...
		allowedRanges, err := parseCIDRs(s.AllowedIPRanges)
		if err != nil {
			return nil, fmt.Errorf("invalid allowedIpRanges: %w", err)
		}

		customBlocked, err := parseCIDRs(s.CustomBlockedIPRanges)
		if err != nil {
		...
		if ip := net.ParseIP(parsedURL.Hostname()); ip != nil {
			if guard.IsIPBlocked(ip) {
				return nil, fmt.Errorf("invalid BaseURL %s: points to a blocked internal IP address", s.BaseURL)
			}
		}
...
// HttpDefaultHeaders reports the resolved default headers if the client has
// been built. It is the type discriminator tools assert on; callers that need
// the resolved headers must use HttpDefaultHeadersContext, since a deferred
// source has not built its client yet.
...
// HttpDefaultHeadersContext returns the default headers, building the client
// on first use so the User-Agent is filled in.
...
// Client reports the HTTP client if one has been built. A deferred source has
// not built one yet; callers that need a client should use RunRequest.
