...
func (s *Source) pool(ctx context.Context) (*sql.DB, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*sql.DB, error) {
		pool, err := initTrinoConnectionPool(ctx, s.tracer, s.Name, s.Host, s.Port, s.User, s.Password, s.Catalog, s.Schema, s.QueryTimeout, s.AccessToken, s.KerberosEnabled, s.SSLEnabled, s.SSLCertPath, s.SSLCert, s.DisableSslVerification)
		if err != nil {
...
// TrinoDB reports the connection pool if one has been made. It is the type
