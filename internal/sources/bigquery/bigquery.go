...
// normalizeAllowedDatasets expands each configured entry to a fully qualified
// "project.dataset" id. This is pure config work, which is why it runs at
// construction: the dataset guard and the tool schemas that advertise the
// allowed names must hold on a source that has not connected. Confirming the
// datasets exist needs a client and happens on connect instead.
...
// clientSet groups the handles this source builds in one connect. Exactly one
// of client/restService and clientCreator is populated: with ADC the source
// owns a client, while under client OAuth every request builds its own from
// the caller's token.
...
// clients returns the BigQuery clients, creating them on first use.
func (s *Source) clients(ctx context.Context) (*clientSet, error) {
	return s.conn.Do(ctx, func(ctx context.Context) (*clientSet, error) {
		...
		if !usesClientOAuth(r) {
			...
			client, restService, tokenSource, err := initBigQueryConnection(ctx, s.tracer, r.Name, r.Project, r.Location, r.QuotaProject, r.ImpersonateServiceAccount, r.Scopes, endpoint)
			if err != nil {
			...
			// use client OAuth
			baseClientCreator, err := newBigQueryClientCreator(ctx, s.tracer, r.Project, r.Location, r.QuotaProject, r.Name, endpoint)
			if err != nil {
		...
		// Verify the allowed datasets exist. A client-OAuth source has no
		// credentials of its own, so it cannot check.
		if cs.client != nil {
...
// newClientCaches allocates the per-token client caches. They are built at
// construction rather than on connect because the Dataplex creator reads the
// dataplex cache without going through the BigQuery connect.
...
// cachingClientCreator wraps baseCreator so that repeat calls with the same
// token reuse the client it built.
...
// BigQueryClient reports the ADC client if one has been made. It is nil under
// client OAuth, and nil on a deferred source that has not connected yet;
// callers that need a client must use RetrieveClientAndService.
...
// BigQueryTokenSource reports the ADC token source if the source has
// connected; a deferred source has not looked up credentials yet.
...
// BigQueryClientCreator reports the per-token client creator if the source has
// connected. It is nil with ADC, where the source owns a single client.
