...
{{< notice note >}}
Secure parameters are supported starting in `toolbox-llamaindex` version `0.9.0` (with `toolbox-core` >= `1.4.0`) and require MCP protocol version `2026-07-28` or newer with the `com.google.cloud/toolbox.v1` extension.
{{< /notice >}}
...
* **Schema Isolation:** Secure parameters are automatically omitted from LlamaIndex's tool metadata and parameter definitions, keeping model context clean and preventing parameter hallucination or leakage.
