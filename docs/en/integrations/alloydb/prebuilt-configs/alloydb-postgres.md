...
*   **Environment Variables:**
    ...
    *   `ALLOYDB_POSTGRES_READONLY`: (Optional) When set to `true`, enforces read-only execution at the database session level (`alloydb_session_read_only=locked`) and suppresses write-capable tools. If you encounter an unsupported parameter error, make sure your AlloyDB instance has the latest Postgres version. Default: `false`.
*   **Permissions:**
